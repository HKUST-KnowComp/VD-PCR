...
class BertConfigHead(object):
    ...
    def __init__(
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (
            ...
            self.heads_for_coref = {num_hidden_layers - 1: [i for i in range(num_attention_heads)]}
        else:
            raise ValueError(
    ...
    def from_dict(cls, json_object):
        ...
        config = BertConfigHead(vocab_size_or_config_json_file=-1)
        for key, value in json_object.items():
            config.__dict__[key] = value

        config.heads_for_coref = {h['layer']: h['heads'] for h in config.heads_for_coref}
        for layer, heads in config.heads_for_coref.items():
            assert 0 <= layer < config.num_hidden_layers
            assert len(heads) > 0
            for head in heads:
                assert 0 <= head < config.num_attention_heads
        return config
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous() # [batch_size, seq_len, num_head, emb]
        # get head embedding here
        if self.heads is not None:
            head_embedding = context_layer[:, :, self.heads].reshape(context_layer.size(0),
                                                                     context_layer.size(1), -1)
        else:
            head_embedding = None
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape) # [batch_size, seq_len, num_head * emb]
        
        return context_layer, attention_probs, head_embedding
...
class BertAttention(nn.Module):
    def __init__(self, config, heads=None):
        ...
        self.self = BertSelfAttention(config, heads)
        self.output = BertSelfOutput(config)

    def forward(self, input_tensor, attention_mask):
        self_output, attention_probs, head_embedding = self.self(input_tensor, attention_mask)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, attention_probs, head_embedding
...
class BertLayer(nn.Module):
    def __init__(self, config, heads=None):
        ...
        self.attention = BertAttention(config, heads)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)

    def forward(self, hidden_states, attention_mask):
        attention_output, attention_probs, head_embedding = self.attention(hidden_states, attention_mask)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        return layer_output, attention_probs, head_embedding
...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        for layer_id in range(config.num_hidden_layers):
            if layer_id in config.heads_for_coref:
                self.layer.append(BertLayer(config, heads=config.heads_for_coref[layer_id]))
            else:
    ...
    def forward(
        ...
        for v_layer_id, t_layer_id in zip(self.v_biattention_id, self.t_biattention_id):
            ...
            for idx in range(t_start, self.fixed_t_layer):
                with torch.no_grad():
                    txt_embedding, txt_attention_probs, head_embedding_layer = self.layer[idx](txt_embedding, txt_attention_mask)
                    t_start = self.fixed_t_layer
                    if head_embedding_layer is not None:
                        head_embedding.append(head_embedding_layer)
                    if output_all_attention_masks:
        ...
        head_embedding = torch.cat(head_embedding, dim=2) # [batch_size, seq_len, emb]

        return all_encoder_layers_t, all_encoder_layers_v, (all_attention_mask_t, all_attnetion_mask_v, all_attention_mask_c), head_embedding
...
class BertPreTrainedModel(nn.Module):
    ...
    def __init__(self, config, device='cuda:0', default_gpu=True, *inputs, **kwargs):
        ...
        if not isinstance(config, BertConfigHead):
            raise ValueError(
                "Parameter config in `{}(config)` should be an instance of class `BertConfigHead`. "
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        encoded_layers_t, encoded_layers_v, all_attention_mask, head_embedding = self.encoder(
            embedding_output,
            v_embedding_output,
        ...
        return encoded_layers_t, encoded_layers_v, pooled_output_t, pooled_output_v, all_attention_mask, head_embedding
...
class BertForMultiModalPreTrainingHead(BertPreTrainedModel):
    """BERT model with multi modal pre-training heads.
    """

    def __init__(self, config, device, use_apex=False):
        super(BertForMultiModalPreTrainingHead, self).__init__(config, device)
    ...
    def forward(
        ...
        sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v, all_attention_mask, head_embedding = self.bert(
            input_ids,
            image_feat,
            image_loc,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None and image_target is not None:
            ...
            return masked_lm_loss.unsqueeze(0), masked_img_loss.unsqueeze(0), next_sentence_loss.unsqueeze(0), head_embedding, prediction_scores_t, seq_relationship_score
        else:
            return prediction_scores_t, prediction_scores_v, seq_relationship_score, head_embedding, all_attention_mask
    ...
    def get_text_embedding(
        ...
        # in this model, we first embed the images.
        _, _, _, _, _, head_embedding = self.bert(
            input_ids,
            image_feat,
            image_loc,
            sep_indices=sep_indices,
        ...
        return head_embedding # [batch_size, num_words, 768]
